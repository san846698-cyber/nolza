...
import type { Category } from "@/lib/games-home";

export default function JumpNav({ categories }: { categories: Category[] }) {
  const { t } = useLocale();
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-col px-4 sm:px-6 py-2 flex gap-1.5 overflow-x-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
        {categories.map((c, i) => {
          ...
          return (
            <a
              ...
              className={[
                "inline-flex items-baseline gap-1.5 px-3 py-1.5 shrink-0",
                "border rounded-full text-[13px] font-medium tracking-tight",
