...
import { getAnimeConfig } from "@/lib/anime-registry";
import { getResultCounts, incrementResult, isStatsEnabled } from "@/lib/kv-stats";

// 범용 테스트 참여 카운터. KV 미설정 시 graceful(enabled:false) — football route 와 무관, 독립.
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const validKey =
    config.results.some((r) => r.key === resultId) || config.hidden?.key === resultId;
  if (!validKey) {
    return NextResponse.json({ error: "invalid resultId" }, { status: 400 });
  }
  try {
    await incrementResult(testId, resultId);
    return NextResponse.json(withTotal(await getResultCounts(testId)));
  } catch {
    return NextResponse.json(EMPTY);
  }
