...
export default function JoseonCouplePage(): ReactElement {
  ...
  const handleShare = useCallback(async () => {
    ...
    const koShareTitle = `${result.p1.original}와(과) ${result.p2.original}의 조선시대 인연 결과`;
    const shareTitle = t(
      polishKoreanParticlesKo(koShareTitle, [result.p1.original, result.p2.original]),
      `${result.p1.original} and ${result.p2.original}'s Joseon Couple Result`,
    );
      ...
      <ScrollFrame>
        ...
        {copied ? (
          ...
          >
            {t(
              "결과 링크가 복사됐어요!\n친구에게 보내고 반응을 확인해보세요.",
              "Link copied. Send it to a friend and see how they react.",
            )}
          </p>
        ) : null}
