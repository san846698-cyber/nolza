...
export default function HansandoGame({
  initialPicks,
}: {
  initialPicks?: Record<string, string>;
}) {
  const { t } = useLocale();
  const hasInitial = !!initialPicks && Object.keys(initialPicks).length === HANSANDO_DILEMMAS.length;
  ...
  const select = (dilemma: Dilemma, choice: Choice) => {
    ...
    if (step + 1 < HANSANDO_DILEMMAS.length) {
      setStep(step + 1);
    } else {
      setPhase("result");
    }
  ...
  const handleShare = async () => {
    ...
    const url = `${origin}/games/battle-what-if/hansando?p=${code}`;
    const text = t(
      `이순신이라면 — 나의 지휘 유형: ${name} · 이순신과 ${result.commanderMatchCount}/5 일치\n${url}`,
      `If You Were Yi Sun-sin — my command type: ${name} · matched Yi Sun-sin ${result.commanderMatchCount}/5\n${url}`,
    );
    try {
      ...
      if (
        ...
        await (
          ...
          title: t("이순신이라면", "If You Were Yi Sun-sin"),
          text: t(
            `나의 지휘 유형: ${name} · 이순신과 ${result.commanderMatchCount}/5 일치`,
            `My command type: ${name} · matched Yi Sun-sin ${result.commanderMatchCount}/5`,
          ),
          url,
        });
        return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {phase === "dilemma" && (
          <DilemmaScreen
            key={HANSANDO_DILEMMAS[step].id}
            dilemma={HANSANDO_DILEMMAS[step]}
            onSelect={select}
          />
        )}
        {phase === "result" && result && (
...
function IntroScreen({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className="text-center fade-in">
      ...
      >
        {t("1592년 7월 · 견내량", "July 1592 · Gyeonnaeryang Strait")}
      </div>
      <h1
        style={{
          marginTop: 24,
      ...
      >
        {t("이순신이라면", "If You Were Yi Sun-sin")}
      </h1>
      <div
        style={{
          marginTop: 12,
      ...
      >
        {t(
          "임진왜란 발발 두 달 반. 일본 수군 73척이 견내량 좁은 해협에 들어와 있고, 조선 함대는 56척. 도요토미는 수륙합공을 명령했다. 당신은 그가 아는 사실만 알고 있다 — 척후 보고, 사료에 남은 발언, 난중일기의 기록. 다섯 번의 결정이 끝나면 당신의 지휘 성향이 이순신·권율·류성룡 같은 실존 지휘관 유형과 비교된다.",
          "Two and a half months into the Imjin War. 73 Japanese ships sit in the narrow Gyeonnaeryang strait; the Joseon fleet has 56. Hideyoshi has ordered a combined land-sea advance. You know only what Yi Sun-sin knew — scout reports, words attested in his *War Diary*. After five decisions, your command style is matched to a real commander archetype — Yi Sun-sin, Gwon Yul, Ryu Seong-ryong, and others.",
        )}
      </p>
      <p
        style={{
      ...
      >
        {t(
          "각 결정의 브리핑·이순신의 실제·후일담은 『난중일기』·『징비록』(류성룡)·『이충무공전서』·이민웅(2004)·Hawley(2005) 분석에 근거합니다. 정답은 없습니다 — 당신의 손이 어떤 손인지를 비춰줄 뿐입니다.",
          "Briefings, Yi Sun-sin's actual decisions, and shadow outcomes are grounded in his *War Diary*, Ryu Seong-ryong's *Jingbirok*, the *Yi Chungmugong Jeonseo*, Lee Min-woong (2004), and Samuel Hawley (2005). There is no right answer — the game only mirrors what kind of hand yours is.",
        )}
      </p>
...
function DilemmaScreen({
  ...
  return (
    <div className="fade-in">
      ...
      `}</style>
      <div className="hansando-dilemma-grid">
          {/* LEFT — scene + briefing */}
          <div>
            <p
              style={{
...
function ResultScreen({
  ...
  const { t } = useLocale();
  const code = encodePicks(picks);
  const shareCardUrl = `/games/battle-what-if/hansando/share-card?p=${code}`;
...
// sources fields in lib/hansando.ts — no new historical claims invented.
function HistoryTimeline({
  ...
  const opener =
    commanderMatchCount === 5
      ? t(
          "다섯 결정 모두 이순신과 같았다. 사료가 기록한 그대로의 길이 당신의 길 — 매 순간 같은 무게의 결정을 내린 사람의 일관성.",
          "All five decisions matched Yi Sun-sin. The road the sources record is your road — the consistency of someone who weighed every fork the same way.",
        )
      : commanderMatchCount === 0
        ? t(
            "다섯 결정 모두 이순신과 다른 길을 골랐다. 사료에 없는 한산도가 당신의 한산도 — 자기만의 시선으로 갈림길을 읽어낸 결과.",
            "All five diverged from Yi Sun-sin. The Hansando the sources never wrote is yours — the result of reading every fork through your own eyes.",
        )
        : t(
            `다섯 갈림길 중 ${commanderMatchCount}번은 이순신과 같은 결정, ${5 - commanderMatchCount}번은 다른 결정이었다. 그 차이가 곧 당신의 성격이고 판단력이다.`,
            `Of the five forks, ${commanderMatchCount} matched Yi Sun-sin and ${5 - commanderMatchCount} did not. That difference is your personality and your judgment.`,
        );
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {matched ? t("✓ 이순신과 같음", "✓ matched Yi Sun-sin") : t("· 다른 결정", "· different decision")}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                {/* Counterpoint — what Yi Sun-sin actually did, only when differed */}
                {!matched && (
                  <div
                    style={{
                      marginTop: 12,
                  ...
                  >
                    ...
                    >
                      {t("사료가 기록한 이순신", "Yi Sun-sin in the sources")}
                    </div>
                    <div
                      style={{
                        fontSize: 13,
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <span>{t("이순신이라면", "If You Were Yi Sun-sin")}</span>
          <span style={{ color: DIM }}>
        ...
        >
          <div>
            ...
            >
              {t("이순신과의 일치", "Match with Yi Sun-sin")}
            </div>
            <div
              style={{
                marginTop: 2,
