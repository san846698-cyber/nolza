...
export default function HannibalGame({
  initialPicks,
}: {
  initialPicks?: Record<string, string>;
}) {
  const { t } = useLocale();
  const hasInitial = !!initialPicks && Object.keys(initialPicks).length === HANNIBAL_DILEMMAS.length;
  ...
  const select = (dilemma: Dilemma, choice: Choice) => {
    ...
    if (step + 1 < HANNIBAL_DILEMMAS.length) {
      setStep(step + 1);
    } else {
      setPhase("result");
    }
  ...
  const handleShare = async () => {
    ...
    const url = `${origin}/games/battle-what-if/hannibal?p=${code}`;
    const text = t(
      `한니발이라면 — 나의 지휘 유형: ${name} · 한니발과 ${result.commanderMatchCount}/5 일치\n${url}`,
      `If You Were Hannibal — my command type: ${name} · matched Hannibal ${result.commanderMatchCount}/5\n${url}`,
    );
    try {
      ...
      if (
        ...
        await (
          ...
          title: t("한니발이라면", "If You Were Hannibal"),
          text: t(
            `나의 지휘 유형: ${name} · 한니발과 ${result.commanderMatchCount}/5 일치`,
            `My command type: ${name} · matched Hannibal ${result.commanderMatchCount}/5`,
          ),
          url,
        });
        return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {phase === "dilemma" && (
          <DilemmaScreen
            key={HANNIBAL_DILEMMAS[step].id}
            dilemma={HANNIBAL_DILEMMAS[step]}
            onSelect={select}
          />
        )}
        {phase === "result" && result && (
...
function IntroScreen({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className="text-center fade-in">
      ...
      >
        {t("기원전 218–216년 · 이탈리아", "218–216 BCE · Italy")}
      </div>
      <h1
        style={{
          marginTop: 24,
      ...
      >
        {t(
          "알프스 횡단부터 칸나에 직후까지, 한니발이 실제로 마주한 다섯 번의 갈림길을 그대로 가져왔다. 당신은 그가 아는 사실만 알고 있다 — 정찰 보고, 측근 의견, 사료에 남은 발언. 다섯 번의 결정이 끝나면 당신의 지휘 성향이 한니발·파비우스·스키피오 같은 실존 지휘관 유형과 비교된다.",
          "From the Alps crossing to the moment after Cannae — five forks Hannibal actually faced, brought to you as he saw them. You know only what he knew: scout reports, council voices, words attested in the sources. After your five decisions, your command style is matched to a real commander archetype — Hannibal, Fabius, Scipio, and others.",
        )}
      </p>
      <p
        style={{
      ...
      >
        {t(
          "각 결정의 브리핑·한니발의 실제·후일담은 폴리비오스·리비우스 1차 사료와 Goldsworthy(2000)·Lazenby(1978)·Daly(2002) 분석에 근거합니다. 정답은 없습니다 — 당신의 손이 어떤 손인지를 비춰줄 뿐입니다.",
          "Briefings, Hannibal's actual decisions, and shadow outcomes are grounded in Polybius and Livy as primary sources, and modern analyses by Goldsworthy (2000), Lazenby (1978), and Daly (2002). There is no right answer — the game only mirrors what kind of hand yours is.",
        )}
      </p>
...
function ResultScreen({
  ...
  const { t } = useLocale();
  const code = encodePicks(picks);
  const shareCardUrl = `/games/battle-what-if/hannibal/share-card?p=${code}`;
...
// sources fields in lib/hannibal.ts — no new historical claims invented.
function HistoryTimeline({
  ...
  const opener =
    commanderMatchCount === 5
      ? t(
          "다섯 결정 모두 한니발과 같았다. 사료가 기록한 그대로의 길이 당신의 길 — 매 순간 같은 무게의 결정을 내린 사람의 일관성.",
          "All five decisions matched Hannibal. The road the sources record is your road — the consistency of someone who weighed every fork the same way.",
        )
      : commanderMatchCount === 0
        ? t(
            "다섯 결정 모두 한니발과 다른 길을 골랐다. 사료에 없는 카르타고가 당신의 카르타고 — 자기만의 시선으로 갈림길을 읽어낸 결과.",
            "All five diverged from Hannibal. The Carthage the sources never wrote is yours — the result of reading every fork through your own eyes.",
        )
        : t(
            `다섯 갈림길 중 ${commanderMatchCount}번은 한니발과 같은 결정, ${5 - commanderMatchCount}번은 다른 결정이었다. 그 차이가 곧 당신의 성격이고 판단력이다.`,
            `Of the five forks, ${commanderMatchCount} matched Hannibal and ${5 - commanderMatchCount} did not. That difference is your personality and your judgment.`,
        );
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {matched ? t("✓ 한니발과 같음", "✓ matched Hannibal") : t("· 다른 결정", "· different decision")}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                {/* Counterpoint — what Hannibal actually did, only when differed */}
                {!matched && (
                  <div
                    style={{
                      marginTop: 12,
                  ...
                  >
                    ...
                    >
                      {t("사료가 기록한 한니발", "Hannibal in the sources")}
                    </div>
                    <div
                      style={{
                        fontSize: 13,
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <span>{t("한니발이라면", "If You Were Hannibal")}</span>
          <span style={{ color: DIM }}>
        ...
        >
          <div>
            ...
            >
              {t("한니발과의 일치", "Match with Hannibal")}
            </div>
            <div
              style={{
                marginTop: 2,
