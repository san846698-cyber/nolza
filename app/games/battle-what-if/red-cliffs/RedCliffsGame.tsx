...
export default function RedCliffsGame({
  initialPicks,
}: {
  initialPicks?: Record<string, string>;
}) {
  const { t } = useLocale();
  const hasInitial = !!initialPicks && Object.keys(initialPicks).length === RED_CLIFFS_DILEMMAS.length;
  ...
  const select = (dilemma: Dilemma, choice: Choice) => {
    ...
    if (step + 1 < RED_CLIFFS_DILEMMAS.length) {
      setStep(step + 1);
    } else {
      setPhase("result");
    }
  ...
  const handleShare = async () => {
    ...
    const url = `${origin}/games/battle-what-if/red-cliffs?p=${code}`;
    const text = t(
      `주유라면 — 나의 지휘 유형: ${name} · 주유와 ${result.commanderMatchCount}/5 일치\n${url}`,
      `If You Were Zhou Yu — my command type: ${name} · matched Zhou Yu ${result.commanderMatchCount}/5\n${url}`,
    );
    try {
      ...
      if (
        ...
        await (
          ...
          title: t("주유라면", "If You Were Zhou Yu"),
          text: t(
            `나의 지휘 유형: ${name} · 주유와 ${result.commanderMatchCount}/5 일치`,
            `My command type: ${name} · matched Zhou Yu ${result.commanderMatchCount}/5`,
          ),
          url,
        });
        return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {phase === "dilemma" && (
          <DilemmaScreen
            key={RED_CLIFFS_DILEMMAS[step].id}
            dilemma={RED_CLIFFS_DILEMMAS[step]}
            onSelect={select}
          />
        )}
        {phase === "result" && result && (
...
function IntroScreen({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className="text-center fade-in">
      ...
      >
        {t("208년 겨울 · 장강", "Winter 208 CE · River Yangtze")}
      </div>
      <h1
        style={{
          marginTop: 24,
      ...
      >
        {t(
          "조조의 자칭 80만 군이 장강 북안에 진을 쳤다. 손권의 조정이 항복론과 결전론으로 갈라지고, 동남풍은 며칠뿐이다. 당신은 그가 아는 사실만 알고 있다 — 정찰 보고, 측근 의견, 정사에 남은 논의. 다섯 번의 결정이 끝나면 당신의 지휘 성향이 주유·노숙·제갈량 같은 실존 지휘관 유형과 비교된다.",
          "Cao Cao's claimed 800,000 are camped on the north bank of the Yangtze. Sun Quan's court is split between surrender and battle, and the southeast wind blows only a few days. You know only what Zhou Yu knew — scout reports, council voices, words attested in the official histories. After five decisions, your command style is matched to a real commander archetype — Zhou Yu, Lu Su, Zhuge Liang, and others.",
        )}
      </p>
      <p
        style={{
      ...
      >
        {t(
          "각 결정의 브리핑·주유의 실제·후일담은 진수 『삼국지·주유전』·사마광 『자치통감』·de Crespigny(1990) 분석에 근거합니다. 정답은 없습니다 — 당신의 손이 어떤 손인지를 비춰줄 뿐입니다.",
          "Briefings, Zhou Yu's actual decisions, and shadow outcomes are grounded in Chen Shou's *Sanguozhi*, Sima Guang's *Zizhi Tongjian*, and Rafe de Crespigny (1990). There is no right answer — the game only mirrors what kind of hand yours is.",
        )}
      </p>
...
function DilemmaScreen({
  ...
  return (
    <div className="fade-in">
      ...
      `}</style>
      <div className="red-cliffs-dilemma-grid">
          {/* LEFT — scene + briefing */}
          <div>
            <p
              style={{
...
function ResultScreen({
  ...
  const { t } = useLocale();
  const code = encodePicks(picks);
  const shareCardUrl = `/games/battle-what-if/red-cliffs/share-card?p=${code}`;
...
// sources fields in lib/red-cliffs.ts — no new historical claims invented.
function HistoryTimeline({
  ...
  const opener =
    commanderMatchCount === 5
      ? t(
          "다섯 결정 모두 주유와 같았다. 사료가 기록한 그대로의 길이 당신의 길 — 매 순간 같은 무게의 결정을 내린 사람의 일관성.",
          "All five decisions matched Zhou Yu. The road the sources record is your road — the consistency of someone who weighed every fork the same way.",
        )
      : commanderMatchCount === 0
        ? t(
            "다섯 결정 모두 주유와 다른 길을 골랐다. 사료에 없는 적벽이 당신의 적벽 — 자기만의 시선으로 갈림길을 읽어낸 결과.",
            "All five diverged from Zhou Yu. The Red Cliffs the sources never wrote is yours — the result of reading every fork through your own eyes.",
        )
        : t(
            `다섯 갈림길 중 ${commanderMatchCount}번은 주유와 같은 결정, ${5 - commanderMatchCount}번은 다른 결정이었다. 그 차이가 곧 당신의 성격이고 판단력이다.`,
            `Of the five forks, ${commanderMatchCount} matched Zhou Yu and ${5 - commanderMatchCount} did not. That difference is your personality and your judgment.`,
        );
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {matched ? t("✓ 주유와 같음", "✓ matched Zhou Yu") : t("· 다른 결정", "· different decision")}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                {/* Counterpoint — what Zhou Yu actually did, only when differed */}
                {!matched && (
                  <div
                    style={{
                      marginTop: 12,
                  ...
                  >
                    ...
                    >
                      {t("사료가 기록한 주유", "Zhou Yu in the sources")}
                    </div>
                    <div
                      style={{
                        fontSize: 13,
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <span>{t("주유라면", "If You Were Zhou Yu")}</span>
          <span style={{ color: DIM }}>
        ...
        {/* Axes 2x2 mini-grid */}
        <div
          className="red-cliffs-share-axes"
          style={{
            marginTop: 22,
            display: "grid",
