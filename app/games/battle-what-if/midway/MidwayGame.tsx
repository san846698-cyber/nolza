...
export default function MidwayGame({
  initialPicks,
}: {
  initialPicks?: Record<string, string>;
}) {
  const { t } = useLocale();
  const hasInitial = !!initialPicks && Object.keys(initialPicks).length === MIDWAY_DILEMMAS.length;
  ...
  const select = (dilemma: Dilemma, choice: Choice) => {
    ...
    if (step + 1 < MIDWAY_DILEMMAS.length) {
      setStep(step + 1);
    } else {
      setPhase("result");
    }
  ...
  const handleShare = async () => {
    ...
    const url = `${origin}/games/battle-what-if/midway?p=${code}`;
    const text = t(
      `니미츠라면 — 나의 지휘 유형: ${name} · 니미츠와 ${result.commanderMatchCount}/5 일치\n${url}`,
      `If You Were Nimitz — my command type: ${name} · matched Nimitz ${result.commanderMatchCount}/5\n${url}`,
    );
    try {
      ...
      if (
        ...
        await (
          ...
          title: t("니미츠라면", "If You Were Nimitz"),
          text: t(
            `나의 지휘 유형: ${name} · 니미츠와 ${result.commanderMatchCount}/5 일치`,
            `My command type: ${name} · matched Nimitz ${result.commanderMatchCount}/5`,
          ),
          url,
        });
        return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {phase === "dilemma" && (
          <DilemmaScreen
            key={MIDWAY_DILEMMAS[step].id}
            dilemma={MIDWAY_DILEMMAS[step]}
            onSelect={select}
          />
        )}
        {phase === "result" && result && (
...
function IntroScreen({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className="text-center fade-in">
      ...
      >
        {t("1942년 6월 · 태평양", "June 1942 · Pacific")}
      </div>
      <h1
        style={{
          marginTop: 24,
      ...
      >
        {t(
          "진주만 6개월 후. 산호해에서 항모 한 척을 잃었다. 일본이 'AF'라는 코드명의 다음 큰 한 수를 준비한다는 정보가 들어온다 — 미군 항모 3척 vs 일본 4척. 당신은 그가 아는 사실만 알고 있다 — 정보국 보고, 참모의 의견, 사료에 남은 명령서. 다섯 번의 결정이 끝나면 당신의 지휘 성향이 니미츠·스프루언스·할시 같은 실존 지휘관 유형과 비교된다.",
          "Six months after Pearl Harbor. You've just lost a carrier at Coral Sea. Intelligence says Japan is preparing its next big move under the codename 'AF' — three U.S. carriers against four Japanese. You know only what Nimitz knew — Station Hypo's reports, staff voices, the orders attested in the record. After five decisions, your command style is matched to a real commander archetype — Nimitz, Spruance, Halsey, and others.",
        )}
      </p>
      <p
        style={{
      ...
      >
        {t(
          "각 결정의 브리핑·니미츠의 실제·후일담은 Walter Lord(1967)·Lundstrom(1984)·Parshall & Tully(2005)·Symonds(2011) 분석에 근거합니다. 정답은 없습니다 — 당신의 손이 어떤 손인지를 비춰줄 뿐입니다.",
          "Briefings, Nimitz's actual decisions, and shadow outcomes are grounded in Walter Lord (1967), Lundstrom (1984), Parshall & Tully (2005), and Symonds (2011). There is no right answer — the game only mirrors what kind of hand yours is.",
        )}
      </p>
...
function DilemmaScreen({
  ...
  return (
    <div className="fade-in">
      ...
      `}</style>
      <div className="hannibal-dilemma-grid">
          {/* LEFT — scene + briefing */}
          <div>
            <p
              style={{
...
function ResultScreen({
  ...
  const { t } = useLocale();
  const code = encodePicks(picks);
  const shareCardUrl = `/games/battle-what-if/midway/share-card?p=${code}`;
...
// sources fields in lib/midway.ts — no new historical claims invented.
function HistoryTimeline({
  ...
  const opener =
    commanderMatchCount === 5
      ? t(
          "다섯 결정 모두 니미츠와 같았다. 사료가 기록한 그대로의 길이 당신의 길 — 매 순간 같은 무게의 결정을 내린 사람의 일관성.",
          "All five decisions matched Nimitz. The road the sources record is your road — the consistency of someone who weighed every fork the same way.",
        )
      : commanderMatchCount === 0
        ? t(
            "다섯 결정 모두 니미츠와 다른 길을 골랐다. 사료에 없는 미드웨이가 당신의 미드웨이 — 자기만의 시선으로 갈림길을 읽어낸 결과.",
            "All five diverged from Nimitz. The Midway the sources never wrote is yours — the result of reading every fork through your own eyes.",
        )
        : t(
            `다섯 갈림길 중 ${commanderMatchCount}번은 니미츠와 같은 결정, ${5 - commanderMatchCount}번은 다른 결정이었다. 그 차이가 곧 당신의 성격이고 판단력이다.`,
            `Of the five forks, ${commanderMatchCount} matched Nimitz and ${5 - commanderMatchCount} did not. That difference is your personality and your judgment.`,
        );
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  {matched ? t("✓ 니미츠와 같음", "✓ matched Nimitz") : t("· 다른 결정", "· different decision")}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                {/* Counterpoint — what Nimitz actually did, only when differed */}
                {!matched && (
                  <div
                    style={{
                      marginTop: 12,
                  ...
                  >
                    ...
                    >
                      {t("사료가 기록한 니미츠", "Nimitz in the sources")}
                    </div>
                    <div
                      style={{
                        fontSize: 13,
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <span>{t("니미츠라면", "If You Were Nimitz")}</span>
          <span style={{ color: DIM }}>
